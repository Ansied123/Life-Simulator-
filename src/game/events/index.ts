...
import { schoolEventCandidates } from './school';
...
export function pickEvent(c: Character, firedOnce: Set<string>): GameEvent | null {
  const candidates = ALL_EVENTS.filter((e) => e.condition(c))
    .concat(familyEventCandidates(c))
    .concat(schoolEventCandidates(c));
